...
def buildSearchRequestData(data):
    ...
    return json.dumps(
        {
            "criteria": {
                k: ("null" if v is None else v) for k, v in data["criteria"].items()
            }
        }
    ).replace('"null"', "null")
...
def doSearchRequest(searchRequestData):
    ret = requests.post(
        SEARCH_URL,
        data=buildSearchRequestData(searchRequestData),
...
def buildFileInfo(soundInfo):
    ...
    catagoryString = (
        ", ".join([c["className"] for c in soundInfo["categories"]]) or "Uncatagorized"
    )
    descriptionString = soundInfo["description"]
    tagString = ", ".join(soundInfo["tags"])
    additionalData = "\t" + "\n\t".join(
        [f"{k} - {v}" for k, v in soundInfo["additionalMetadata"].items() if v]
    )

    # print(f"TITLE: {catagoryString} - {soundIDString}")
    # print(f"TAGS:\n\t{tagString}\nDESCRIPTION:\n\t{descriptionString}\nMETADATA:\n{additionalData}")
    # print("\n\n\n")
    return f"{descriptionString}\nTAGS:\t{tagString}\nMETADATA:\n{additionalData}"


def buildSearchFilters():
    for filter_type in filters:
        ret = requests.post(
            filters[filter_type]["url"],
            headers={"Content-Type": "application/json"},
            data=buildSearchRequestData(searchRequestData),
        )
        ret.raise_for_status()
        ...
        filters[filter_type]["values"] = {
            k: v["doc_count"] for k, v in parsedRet["aggregations"].items()
        }
...
def doFileDownload(soundInfo, outputFolder, fileType):
    ...
    if fileType == "flac":
        ...
        with requests.get(
            f"https://sound-effects-media.bbcrewind.co.uk/wav/{soundIDString}.wav",
            stream=True,
        ) as request:
        ...
        try:
            ...
            AudioSegment.from_wav(tmpFilePath).export(
                flacPath,
                format="flac",
                parameters=["-compression_level", str(FLAC_COMPRESSION_LEVEL)],
                tags={"Comments": buildFileInfo(soundInfo)},
            )
        except Exception:
        ...
        with requests.get(
            f"https://sound-effects-media.bbcrewind.co.uk/{fileType}/{soundIDString}.{fileType}",
            stream=True,
        ) as request:
...
def buildDavinciSoundLibraryDB(allSounds):
    print(
        "Great! first we need to set up the sounds in Davinci. Please refer to the README.md section on doing so, then return here and press enter to continue..."
    )
    input()
    ...
    while not os.path.exists(soundLibDBPath) and not os.path.isfile(soundLibDBPath):
        print(
            "The path you entered was incorrect, make sure you specify the full path, including SoundLib.db"
        )
        soundLibDBPath = input("Enter the path to SoundLib.db: ")
    ...
    for soundInfo in tqdm(allSounds):
        soundIDString = soundInfo["id"]
        catagoryString = (
            ", ".join([c["className"] for c in soundInfo["categories"]])
            or "Uncatagorized"
        )
        ...
        sqlCur.execute(
            f"UPDATE FLAssetBaseClip SET description = ? WHERE name = ?",
            (buildFileInfo(soundInfo), dbNameFieldValue),
        )
    sqlCon.commit()
...
def scrapeBBCSearchAPI(allSounds):
    ...
    for category in (pbar_category := tqdm(filters["category"]["values"], leave=False)):
        ...
        if category != "Nature":
            ...
            deduped_results, non_deduped_search_result_count = doSearchRequest(
                searchRequestData
            )
            ...
            natureSounds = set()
            for duration in (
                pbar_duration := tqdm(filters["duration"]["values"], leave=False)
            ):
                pbar_duration.set_description(" " * 4 * 2 + f"duration: {duration}")

                searchRequestData["criteria"]["durations"] = [
                    {"min": duration.split("-")[0], "max": duration.split("-")[1]}
                ]
            ...
            searchRequestData["criteria"]["durations"] = None
            for continent in (
                pbar_duration := tqdm(filters["continent"]["values"], leave=False)
            ):
        ...
        if category_total_sounds != filters["category"]["values"][category]:
            tqdm.write(
                f"WARNING: missing sounds in {category}! Expected {filters['category']['values'][category]} but only got {category_total_sounds}"
            )
            sleep(2)
...
if __name__ == "__main__":
    ...
    if not USE_PICKEL_SEARCH_DB or not os.path.isfile("./allSounds.pkl"):
        ...
        selected_catagories = input(
            f"Would you like to download a subset of catagories? valid choices are 'all', or a comma seperated list of values from the following choices:\n\t{filterChoiceString}\n"
        )
        ...
        if selected_catagories == "all":
            if len(allSounds) != totalSoundCount:
                print(
                    f"Warning: Missing sounds! expected {totalSoundCount} but only got {len(allSounds)}"
                )
            else:
                print("All sounds scraped sucessfully. Ready to download...")
        else:
            if len(allSounds) < sum(
                filters["category"]["values"].values()
            ):  # If you thought you could just use this sum instead of hitting the API to get the sound count... think again! If you sum all the catagories, you get more than the total the API gives. Probably duplicates?
                print(
                    f"Warning: Missing sounds! expected {sum(filters['category']['values'].values())} but only got {len(allSounds)}"
                )
            else:
                print("All sounds scraped sucessfully. Ready to download...")
    ...
    if not REBUILD_RESOLVE_DB_ONLY:
        ...
        total_download_size_mp3 = sum(
            [int(sound["fileSizes"]["mp3FileSize"]) for sound in allSounds]
        )
        total_download_size_wav = sum(
            [int(sound["fileSizes"]["wavFileSize"]) for sound in allSounds]
        )

        file_format = input(
            f"Would you like wav (raw), mp3 (lossy compression), or flac (lossless compression, will take longer and make your CPU scream)?\n"
        ...
        while file_format not in ["flac", "wav", "mp3"]:
            file_format = input(
                "invalid input... try again, you must input one of the following: [flac, wav, mp3]. If the sizes scare you, press ctrl+c :p\n"
            )
        ...
        while not outputLocationOK:
            ...
            if not os.path.exists(outputLocation) and not os.path.isdir(outputLocation):
                print(
                    "Your output path either does not exist, or is not a directory. try again!"
                )
                continue
            if os.listdir(outputLocation):
                print(
                    f"Warning! the file path entered is not empty! we are about to dump {len(allSounds)} into it! you probably don't want to do this"
                )
        ...
        with tqdm(total=len(allSounds)) as progressBar:
            with ThreadPoolExecutor() as thread_pool:
                futures = [
                    thread_pool.submit(
                        doFileDownload, soundInfo, outputLocation, file_format
                    )
                    for soundInfo in allSounds
                ]
    ...
    buildSoundDBConf = input(
        "Would you like to build a sound library for Davinci Resolve? [yes/no]: "
    )
    ...
    print(
        "Thanks for checking out my goofy project, happy editing! (or happy listening to bird sounds, I don't judge)"
    )
